...
A read that hits the limit reports itself truncated rather than answering as though it were whole, and a
bigger `--limit` is what answers that — see above for why `--include` is not.
