...
> Amended by [0042](./0042-the-origin-read-names-its-directory-and-reads-the-repositorys-own-config.md) on
> two points. "What the prefix does not cover" leaves both config-location vectors open — 0042 closes them,
> though not with either option listed below: it reads every scope and keeps the ones the checkout configures
> for itself, rather than selecting `--local`, which 0042 measures as too narrow. And Consequences defends the
> origin read carrying no `-C` — 0042 gives it one, so "almost every command this tool issues names the
