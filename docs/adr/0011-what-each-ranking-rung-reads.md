...
"A number" means one at or below 2^53-1. Nothing bounds how many digits a label carries, and above that
point distinct digit runs stop mapping to distinct values: `P9007199254740993` and `P9007199254740992`
both read as 9007199254740992, so two priorities a person wrote as different would tie on the rung that
claims to compare numbers. The whole range above the boundary is reported as unread rather than ranked
— not merely the values that round, which would leave the ladder ordering an arbitrary half of it. That
is the same refusal as for a named value and for the same reason: this rung does not guess.
