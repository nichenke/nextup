...
A fixture only ever puts a shape where its author expected one, so `bun run check:live` reads a real
repository through the adapter and checks the answer against the tracker's own account of the same
tickets. Credentialed, local, and never CI, for the same reason the capture is:
`docs/agents/live-invariants.md` has how to run it and what each verdict means.
